  ...
  <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <div className="p-4 border-b border-gray-100"><div className="flex items-center justify-between">
      ...
      <div className="flex items-center flex-shrink-0">
        ...
        <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={safePage === 0} className={"px-2 py-1 text-xs font-medium transition " + (safePage === 0 ? "text-gray-300 cursor-not-allowed" : "text-gray-500 hover:text-gray-700")}>Back</button>
        <span className="text-gray-300 mx-1">·</span>
...
function App() {
  ...
  const allGrouped = useMemo(() => {
    ...
    if (fPid.trim()) { const r = groups.filter(g => g.partners.includes(fPid.trim())); if (r.length) groups = r; }
    return groups;
  ...
  const fo = useMemo(() => {
    ...
    const pidHasResults = !fPid.trim() || allGroups.some(g => g.partners.includes(fPid.trim()));
    const effectivePid = pidHasResults ? fPid.trim() : "";
